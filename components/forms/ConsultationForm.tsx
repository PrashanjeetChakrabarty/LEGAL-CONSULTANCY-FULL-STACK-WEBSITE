...
export default function ConsultationForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // ONE call — backend creates consultation in MySQL + creates Razorpay order
      ...
      const { consultation, orderId, keyId, amount, currency } = data;

      // Load Razorpay SDK & open modal
      ...
      script.onload = () => {
        const rzp = new window.Razorpay({
          ...
          name: "F&V Legal Consultancies",
          description: "Initial Consultation — 45 min",
          order_id: orderId,
          prefill: { name: form.name, email: form.email, contact: form.phone },
          theme: { color: "#D4AF37" },
          handler: async (response: any) => {
            // Verify payment on backend — updates MySQL consultation row to 'paid'
            ...
            if (vRes.ok) { setSuccess(true); setStep(4); }
    ...
    return (
      <div className="w-full max-w-2xl mx-auto rounded-3xl border border-white/10 bg-[#0a0a0a] p-12 text-center">
        ...
        <p className="text-white/40 text-sm max-w-sm mx-auto leading-relaxed">
          Payment confirmed. Your consultation has been recorded and we will reach out to{" "}
  ...
  // ── Step indicator ────────────────────────────────────────────────────────
  const STEPS = ["Personal Details", "Legal Matter", "Review & Pay"];
      ...
      <div className="p-8 md:p-12">
        ...
        <form onSubmit={step === 3 ? handleSubmit : handleNext}>
          <AnimatePresence mode="wait">
            ...
            {step === 3 && (
              <motion.div
                ...
                transition={{ duration: 0.25 }}
              >
                <h3 className="text-xl font-serif text-white mb-1">Review & Payment</h3>
                <p className="text-sm text-white/30 mb-8">Initial consultation fee secures your 45-minute session.</p>
                ...
                <div className="rounded-2xl border border-white/8 bg-black/20 divide-y divide-white/5 mb-6">
                  ...
                  <div className="flex items-center justify-between px-5 py-4 bg-[#D4AF37]/5">
                    <span className="text-xs uppercase tracking-widest text-white/40">Consultation Fee</span>
                    <span className="text-2xl font-serif text-[#D4AF37]">₹2,500</span>
                  </div>
                </div>

                <p className="text-xs text-white/20 leading-relaxed mb-6">
                  By proceeding, you agree to our Terms of Service. Payment is processed securely via Razorpay.
                  Your booking details will be saved to our secure database immediately.
                </p>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <div className="flex justify-between mt-10 pt-6 border-t border-white/5">
            ...
            >
              {step === 3
                ? isSubmitting ? "Processing..." : "Pay ₹2,500 & Book"
                : "Continue →"}
            </button>
          </div>
        </form>
